...
app.controller('goodsController' ,function($scope, $controller, $location, goodsService, uploadService, itemCatService, typeTemplateService){
	...
	$scope.findOne=function(){
		...
		goodsService.findOne(id).success(
			function(response){
				...
				$scope.typeTemplate.customAttributeItems = $scope.entity.itemList.spec;
			}
		);
	}
		...
		goodsService.search(page,rows,$scope.searchEntity).success(
			function(response){
				...
				for (var i = 0; i < response.rows.length; i++){
					if (response.rows[i].isDelete != "1"){
						$scope.list.push(response.rows[i])
						num ++;
					}
				}*/
				$scope.list=response.rows;
	...
	$scope.isCheck = function (specName, optionName) {
		var items = $scope.entity.goodsDesc.specificationItems;
		var object = $scope.searchObjectByKey(items, "attributeName", specName);
		if (object != null){
			if (object.attributeValue.indexOf(optionName) >= 0){
	...
	$scope.updateOneStatus = function (id, status) {
		var idArr = [id];
		goodsService.updateStatus(idArr, status).success(function (resp) {
			if (resp.success){
				$scope.reloadList();
				$scope.selectIds = [];
			}else {
